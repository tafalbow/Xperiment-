...
def test_evaluation_matrix_structure():
    ...
    # API endpoint check
    res = client.get("/api/apbn-eval/matrix?year=2025&category=BELANJA&unit=TRILLION")
    assert res.status_code == 200
    b_data = res.json()
...
def test_trajectory_series():
    """Verify S-Curve trajectory coordinates."""
...
def test_export_excel_and_csv():
    """Verify Excel .xlsx (3 sheets) and CSV output generation."""
    # CSV
